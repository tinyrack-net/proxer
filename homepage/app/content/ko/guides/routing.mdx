...
`--subdomain`이 없는 클라이언트는 서버가 할당한 임의 서브도메인을 받습니다.
...
`proxy.example.com` 요청은 이 루트 경로 클라이언트로 갑니다. 루트 경로는 서버를 `--domain`으로 시작했을 때 가장 명확합니다. `--domain`이 없으면 Proxer는 요청 호스트의 첫 번째 label을 서브도메인 경로로 해석합니다.
